...
export default function AdminChatPage() {
    ...
    return (
        <div className="h-[calc(100vh-8rem)] flex bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {filteredConversaciones.map((conv) => (
                        <button
                            ...
                            className={`w-full p-4 flex items-start gap-3 border-b border-slate-100 dark:border-slate-800 transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 ${selectedConv?.id === conv.id ? 'bg-emerald-50 dark:bg-emerald-900/10' : ''
                                }`}
                        >
                            <div className="relative">
                                ...
                                {conv.unreadCount > 0 && (
                                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full text-white text-xs font-bold flex items-center justify-center border-2 border-white dark:border-slate-900">
                                        {conv.unreadCount}
                                    </span>
                                )}
                            </div>
                            <div className="flex-1 text-left min-w-0">
                                <div className="flex justify-between items-start mb-1">
                                    <span className={`font-semibold text-sm truncate ${selectedConv?.id === conv.id ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-200'}`}>
                                        {conv.usuarioNombre}
                                    </span>
                                    {conv.lastMessageAt && (
                        ...
                        <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 px-4 py-3 bg-slate-100 dark:bg-slate-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all dark:text-white"
                                />
                                <button
