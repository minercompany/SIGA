...
public class CandidatoController {
    ...
    public ResponseEntity<?> actualizar(@PathVariable Long id, @RequestBody Map<String, Object> data) {
        return candidatoRepository.findById(id).map(candidato -> {
            if (data.containsKey("organo")) candidato.setOrgano(Candidato.Organo.valueOf(data.get("organo").toString()));
            if (data.containsKey("tipo")) candidato.setTipo(Candidato.TipoCandidato.valueOf(data.get("tipo").toString()));
            if (data.containsKey("foto")) candidato.setFoto((String) data.get("foto"));
            if (data.containsKey("biografia")) candidato.setBiografia((String) data.get("biografia"));
            if (data.containsKey("orden")) candidato.setOrden((Integer) data.get("orden"));
            if (data.containsKey("activo")) candidato.setActivo((Boolean) data.get("activo"));
